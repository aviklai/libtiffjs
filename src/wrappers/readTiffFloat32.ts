...
export default function readTiffFloat32(privateRegistry) {
  return function (tiffPtr) {
    ...
    const rowsPerStrip = privateRegistry.TIFFGetField(
      tiffPtr,
      TiffTag.ROWSPERSTRIP
    );

    const bytesPerSample =
      privateRegistry.TIFFGetField(tiffPtr, TiffTag.BITSPERSAMPLE) / 8;
    const tiffFloatArray = new Float32Array(width * height);

    const sbuf = privateRegistry.TIFFMalloc(
      privateRegistry.TIFFStripSize(tiffPtr)
    );
    for (let s = 0; s < numStrips; ++s) {
      ...
      const stripData = new Float32Array(
        // @ts-ignore
        HEAPF32.buffer,
        sbuf,
        read / bytesPerSample
